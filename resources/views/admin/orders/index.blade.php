            ...
            <tbody>
                @foreach ($payments as $key => $payment)
                    <tr>
                            ...
                            @if ($payment->enrollment)
                                <form action="{{ route('admin.enrollments.updateStatus', $payment) }}" method="POST">
                                    ...
                                    <select name="enrollment_status" class="form-control form-control-sm">
                                        <option value="active" {{ $payment->enrollment->status === 'active' ? 'selected' : '' }}>Đã đăng ký</option>
                                        <option value="pending" {{ $payment->enrollment->status === 'pending' ? 'selected' : '' }}>Chờ xử lý</option>
                                        <option value="cancelled" {{ $payment->enrollment->status === 'cancelled' ? 'selected' : '' }}>Đã hủy</option>
                                        <option value="failed" {{ $payment->enrollment->status === 'failed' ? 'selected' : '' }}>Thất bại</option>
                                    </select>
                        ...
                        <td>
                            @if ($payment->enrollment && $payment->enrollment->enrolled_at)
                                {{ $payment->enrollment->enrolled_at->format('d/m/Y H:i') }}
                            @else
                                Chưa có
                            @endif
                        </td>
